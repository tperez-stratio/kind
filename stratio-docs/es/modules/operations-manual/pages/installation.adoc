...
En AWS, puedes usar el método de autorización _assume role_ indicando el parámetro `role_arn`. Por ejemplo:
...
==== Operaciones del cluster con assume role en AWS

Para realizar el manejo del cluster a nivel de operación, sigue estos pasos:

. *Instalación de AWS CLI*: Asegúrate de tener la última versión de AWS CLI instalada. Puedes verificarlo con el siguiente comando:
+
[source,bash]
----
aws --version
...
. Exporta las credenciales temporalmente:
+
[source,bash]
----
...
. Actualiza el kubeconfig para el cluster EKS:
+
[source,bash]
----
