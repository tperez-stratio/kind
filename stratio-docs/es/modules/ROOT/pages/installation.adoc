...
+
** Un rol para los workers del cluster en `spec.security.nodes_identity`.
** Un rol para el control-plane en `spec.security.control_plane_identity`.
+
...
* Sistemas operativos certificados
+
En entornos de Azure, se deben utilizar las imágenes proporcionadas por Stratio. Consulta la sección de xref:stratio-generative-ai-data-fabric:ROOT:stratio-generative-ai-data-fabric-artifacts.adoc#_imágenes_para_entornos_cloud[artefactos de __Stratio Generative AI Data Fabric__].
+
...
Refiriéndose al _control-plane_, en EKS no se podrá indicar una imagen, pero Azure no gestionado sí.
