...
NOTE: Los proveedores no gestionados tendrán un _MachineHealthCheck_ para el _control-plane_ y otro para los nodos _worker_, mientras que los gestionados (EKS) sólo tendrán el segundo.
...
Para actualizar la versión de _Stratio Cloud Provisioner_ de 0.2 a 0.3 o de una _patch version_ de la 0.3 a la última stable de 0.3, se debe ejecutar el _script_ _upgrade-provisioner++_++.py_ desde el workspace con el que se instaló el cluster.
...
[source,bash]
----
python3 upgrade-provisioner.py -p <vault_password> --helm-repo <url> --helm-user <username> --helm-password <password> -a
----

Los descriptores _cluster.yaml_ y _secrets.yml_ deben estar presentes en el workspace. Si el descriptor _cluster.yaml_ tiene otro nombre o está en otra ruta se puede especificar con el parámetro '-d'.
