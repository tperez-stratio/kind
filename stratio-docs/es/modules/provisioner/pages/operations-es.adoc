...
Para comunicarnos con el APIserver del cluster creado, necesitaremos el fichero kubeconfig. Éste fichero se obtendrá de forma diferente según el cloud provider utilizado y la gestión del control-plane del cluster.
...
Si bien no forma parte de la operativa de _Stratio KEOS_ propiamente dicha, es importante resaltar la forma de permitir la https://docs.aws.amazon.com/eks/latest/userguide/add-user-role.html[autenticación de otros usuarios en un cluster de EKS] (el usuario creador del cluster está autenticado por defecto).

Para dar permisos de kubernetes-admin en el cluster, agregaremos el ARN del usuario en el ConfigMap indicado a continuación.
