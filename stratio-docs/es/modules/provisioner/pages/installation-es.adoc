...
- Uso de ECR como Docker registry (sin credenciales).
- Uso de VPC y subnets custom (creadas anteriormente). Este apartado no es obligatorio.
- Hay 3 grupos de nodos workers:
...
spec:
  ...
  credentials:
    ...
    github_token: github_pat_11APW..
    docker_registries:
      - url: XXYYZZ.dkr.ecr.region.amazonaws.com/keos
        user: "myuser"
        pass: "mypass"
  k8s_version: v1.24.15
  region: eu-west-1
