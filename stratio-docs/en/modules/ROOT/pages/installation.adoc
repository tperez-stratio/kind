...
For images created with image-builder (*unmanaged Azure*), the following element must be added to the https://github.com/kubernetes-sigs/image-builder/blob/main/images/capi/ansible/roles/node/tasks/main.yml#L55[loop of the kernel parameters] (in _/images/capi/ansible/roles/node/tasks/main.yml_):
...
* Restart the controller (_aws-load-balancer-controller-manager_):
+
[source,console]
----
