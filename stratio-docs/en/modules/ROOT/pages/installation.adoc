...
+
For EKS deployment, you must manually create the _AWSServiceRoleForAmazonEKS_ role and associate it with the _AmazonEKSServiceRolePolicy_ policy (which is created by default in AWS).

* Certified operating systems
+
To ensure the functionality of the EKS managed _control-plane_, it is necessary to use the images provided by Stratio. These can be found in the xref:stratio-generative-ai-data-fabric:ROOT:stratio-generative-ai-data-fabric-artifacts.adoc#_images_for_cloud_environments[_Stratio Generative AI Data Fabric_ artifacts] section of the documentation.
+
The currently recommended operating system for this provider is Ubuntu 22.04.

* CloudFormation
+
WARNING: Suppose you have not previously created the CloudFormation stack or manually created the IAM requirements in the account. In that case, you must set the `spec.security.aws.create_iam` parameter to _true_ (default is _false_).
...
* Certified operating systems
+
In Azure environments, you must use the images provided by Stratio. You can find them in the xref:stratio-generative-ai-data-fabric:ROOT:stratio-generative-ai-data-fabric-artifacts.adoc#_images_for_cloud_environments[_Stratio Generative AI Data Fabric_ artifacts] section of the documentation.
+
...
* Kubernetes version 1.28.x (EKS does not take into account the patch version).
