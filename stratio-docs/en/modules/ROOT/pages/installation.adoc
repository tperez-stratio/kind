...
+
** xref:attachment$stratio-aws-unmanaged-policy.json[Download permanent permissions for AWS unmanaged].
...
+
In the case of GKE, in addition to the above permissions, additional permissions will be required to create clusters.
+
...
* Bastion.
+
The deployment of _Stratio KEOS_ in managed GCP must be done using a bastion that facilitates communication with the cluster of GKE. For this, it is necessary to create a bastion on the same network as the cluster.
