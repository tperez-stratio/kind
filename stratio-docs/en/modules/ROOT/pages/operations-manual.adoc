...
NOTE: EKS will have a _MachineHealthCheck_ for the _worker_ nodes.
...
To upgrade the _Stratio Cloud Provisioner_ version from 0.2 to 0.3 or from a 0.3 patch version to the latest 0.3 stable you must run the script _upgrade-provisioner++_++.py_ from the workspace where the cluster was installed.
...
The _cluster.yaml_ and _secrets.yml_ descriptors must be present in the workspace. If the _cluster.yaml_ descriptor has a different name or is located in a different path, it can be specified using the '-d' parameter.
