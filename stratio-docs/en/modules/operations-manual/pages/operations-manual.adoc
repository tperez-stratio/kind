...
NOTE: In Unmanaged Azure, there will be a _MachineHealthCheck_ for the _control-plane_ and another for the _worker_ nodes, while managed ones (EKS) will only have the second one.
...
NOTE: If you use the `--skip-k8s-intermediate-version` flag, Kubernetes will be upgraded directly from version 1.26.X to 1.28.X in a single step. This is only supported in EKS environments.
...
* It is crucial to exercise caution in production environments, especially in unmanaged Azure. Before performing an upgrade, it is recommended to create a backup of the objects managing the infrastructure and critical services.
