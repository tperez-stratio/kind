...
NOTE: Managed providers (EKS) will only have a _MachineHealthCheck_ for the _worker_ nodes, while unmanaged ones (Azure) will have one for both the _control-plane_ and the _worker_ nodes.
